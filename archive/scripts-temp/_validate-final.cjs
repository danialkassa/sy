...
function* walk(dir, exts) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory() && !['temp-b2b-clone','.git','node_modules','archive'].includes(entry.name)) {
...
const regex = /[^"'\s]*images\/(?:products|unsplach|istock|uploads)\/[^"'\s)]+/g;
...
console.log('Total: ' + total + ', Broken: ' + broken);
