...
(function() {
  ...
  function buildPlainTextBody(name, company) {
    ...
    var lines = [];
    lines.push('Hello Ningbo Siyang Team,');
    lines.push('');
    lines.push((name || 'A customer') + (company ? ' from ' + company : '') + ' has requested a quote for the following items:');
    lines.push('');
    ...
    lines.push('');
    lines.push('Please provide pricing and estimated lead time.');
    lines.push('');
  ...
  function buildMailtoUrl(name, company) {
    ...
    var subject = encodeURIComponent('Quote Request – Ningbo Siyang');
  ...
  function showConfirmation() {
    ...
    body.innerHTML =
      '<div class="flex flex-col items-center justify-center h-full text-center px-6 py-10">' +
        ...
        '</div>' +
        '<h3 class="text-lg font-bold text-white mb-2">Quote Request Prepared!</h3>' +
        '<p class="text-sm text-zinc-400 mb-4">Your quote request has been prepared. Send the email that was opened in your email client, or copy the details and paste them into an email to <span class="text-yellow-400">' + recipient + '</span></p>' +
        '<button onclick="quoteCart.copyQuoteToClipboard()" id="quote-copy-btn" class="inline-flex items-center justify-center h-10 px-5 bg-zinc-800 border border-zinc-700 text-white hover:bg-zinc-700 font-semibold rounded-md transition-colors text-sm mb-3">' +
          ...
          'Copy Quote Details' +
        '</button>' +
        '<span id="quote-copy-feedback" class="text-xs text-green-400 hidden mb-4">Copied to clipboard!</span>' +
        '<button onclick="quoteCart.close()" class="inline-flex items-center justify-center h-10 px-6 bg-yellow-400 text-zinc-900 hover:bg-yellow-300 font-semibold rounded-md transition-colors">Continue Shopping</button>' +
      '</div>';
    footer.innerHTML =
      '<p class="text-center text-xs text-zinc-500">Your cart has been cleared. You can always add items again later.</p>';
  }
      ...
      '<div class="quote-panel absolute top-0 right-0 h-full w-full bg-zinc-950 border-l border-zinc-800 shadow-2xl transform translate-x-full transition-transform duration-300 ease-out flex flex-col" style="max-width:440px">' +
        '<div class="flex items-center justify-between p-5 border-b border-zinc-800 shrink-0">' +
          '<div class="flex items-center gap-3">' +
            ...
            '</div>' +
            '<div>' +
              '<h2 class="font-oswald text-lg font-bold text-white">Quote Cart</h2>' +
              '<p class="text-xs text-zinc-500"><span id="quote-drawer-count">0</span> items</p>' +
            '</div>' +
          '</div>' +
          '<button onclick="quoteCart.close()" aria-label="Close quote cart" class="w-9 h-9 rounded-lg bg-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-zinc-700 transition-colors">' +
  ...
  function render() {
    ...
    if (items.length === 0) {
      body.innerHTML =
        '<div class="flex flex-col items-center justify-center h-full text-center px-6">' +
          ...
          '</div>' +
          '<h3 class="text-lg font-semibold text-white mb-2">Your quote cart is empty</h3>' +
          '<p class="text-sm text-zinc-500 mb-6">Browse our products and add items to request a quote.</p>' +
          '<a href="' + (window.location.pathname.includes('/products/') ? 'index.html' : 'products/index.html') + '" onclick="quoteCart.close()" class="inline-flex items-center justify-center h-10 px-6 bg-yellow-400 text-zinc-900 hover:bg-yellow-300 font-semibold rounded-md transition-colors">Browse Products</a>' +
        '</div>';
      footer.innerHTML = '';
      return;
    }
      ...
      return '<div class="flex gap-3 p-3 rounded-xl bg-zinc-900 border border-zinc-800 mb-3" data-quote-sku="' + item.sku + '">' +
        ...
        '<div class="flex-1 min-w-0">' +
          '<div class="flex justify-between items-start">' +
            ...
            '</div>' +
            '<button onclick="quoteCart.removeItem(\'' + item.sku + '\')" aria-label="Remove ' + item.name + '" class="text-zinc-600 hover:text-red-400 transition-colors ml-2 flex-shrink-0">' +
          ...
          '<div class="flex items-center justify-between mt-2">' +
            ...
            '<div class="flex items-center bg-zinc-800 rounded-lg border border-zinc-700">' +
              '<button onclick="quoteCart.updateQty(\'' + item.sku + '\',' + (item.quantity - 1) + ')" aria-label="Decrease quantity" class="w-8 h-8 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-zinc-700 rounded-l-lg transition-colors">' +
              ...
              '<button onclick="quoteCart.updateQty(\'' + item.sku + '\',' + (item.quantity + 1) + ')" aria-label="Increase quantity" class="w-8 h-8 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-zinc-700 rounded-r-lg transition-colors">' +
    ...
    html +=
      '<div class="mb-4">' +
        '<label class="block text-sm font-medium text-zinc-300 mb-1">Additional Requirements</label>' +
        '<textarea id="quote-notes" rows="3" placeholder="E.g., Need custom packaging, specific plug type, CE marking..." class="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white placeholder-zinc-500 focus:border-yellow-400 focus:ring-1 focus:ring-yellow-400 outline-none"></textarea>' +
      '</div>' +
      '<div class="mt-6 pt-4 border-t border-zinc-800">' +
        '<h3 class="text-sm font-semibold text-white mb-3">Your Contact Information</h3>' +
        ...
        '<div class="space-y-3">' +
          '<div class="grid grid-cols-2 gap-3">' +
            '<div>' +
              '<label for="quote-contact-name" class="block text-xs text-zinc-400 mb-1">Full Name <span class="text-red-400">*</span></label>' +
              '<input type="text" id="quote-contact-name" required class="w-full h-9 px-3 rounded-md bg-zinc-800 border border-zinc-700 text-white text-sm placeholder:text-zinc-600 focus:border-yellow-400 focus:ring-1 focus:ring-yellow-400/20 outline-none" placeholder="John Smith"/>' +
            '</div>' +
            '<div>' +
              '<label for="quote-contact-company" class="block text-xs text-zinc-400 mb-1">Company</label>' +
              '<input type="text" id="quote-contact-company" class="w-full h-9 px-3 rounded-md bg-zinc-800 border border-zinc-700 text-white text-sm placeholder:text-zinc-600 focus:border-yellow-400 focus:ring-1 focus:ring-yellow-400/20 outline-none" placeholder="Company name"/>' +
            '</div>' +
          '</div>' +
          '<div class="grid grid-cols-2 gap-3">' +
            '<div>' +
              '<label for="quote-contact-email" class="block text-xs text-zinc-400 mb-1">Email <span class="text-red-400">*</span></label>' +
              '<input type="email" id="quote-contact-email" required class="w-full h-9 px-3 rounded-md bg-zinc-800 border border-zinc-700 text-white text-sm placeholder:text-zinc-600 focus:border-yellow-400 focus:ring-1 focus:ring-yellow-400/20 outline-none" placeholder="john@example.com"/>' +
            '</div>' +
            '<div>' +
              '<label for="quote-contact-phone" class="block text-xs text-zinc-400 mb-1">Phone</label>' +
              '<input type="tel" id="quote-contact-phone" class="w-full h-9 px-3 rounded-md bg-zinc-800 border border-zinc-700 text-white text-sm placeholder:text-zinc-600 focus:border-yellow-400 focus:ring-1 focus:ring-yellow-400/20 outline-none" placeholder="+1 555 123 4567"/>' +
            '</div>' +
          '</div>' +
          '<div>' +
            '<label for="quote-contact-notes" class="block text-xs text-zinc-400 mb-1">Additional Notes</label>' +
            '<textarea id="quote-contact-notes" rows="2" class="w-full px-3 py-2 rounded-md bg-zinc-800 border border-zinc-700 text-white text-sm placeholder:text-zinc-600 focus:border-yellow-400 focus:ring-1 focus:ring-yellow-400/20 outline-none resize-none" placeholder="Any special requirements, quantity estimates, target delivery date..."></textarea>' +
          '</div>' +
        '</div>' +
      '</div>';
    ...
    footer.innerHTML =
      '<div class="space-y-3">' +
        ...
        '</div>' +
        '<div class="flex justify-between text-sm">' +
          '<span class="text-zinc-400">Unique Products</span>' +
        ...
        '<div class="flex gap-2">' +
          '<button onclick="quoteCart.clear()" class="flex-1 text-sm text-zinc-500 hover:text-red-400 transition-colors py-2">Clear cart</button>' +
          '<button onclick="quoteCart.close()" class="flex-1 text-sm text-zinc-500 hover:text-zinc-300 transition-colors py-2">Continue shopping</button>' +
        '</div>' +
      '</div>';
  }
  ...
  window.quoteCart = {
    ...
    sendRequest: function() {
      ...
      if (sendBtn) {
        ...
        sendBtn.innerHTML = '<svg class="w-5 h-5 mr-2 animate-spin" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke-opacity="0.25"/><path d="M12 2a10 10 0 0 1 10 10" stroke-opacity="1"/></svg>Sending...';
      }
    ...
    copyQuoteToClipboard: function() {
      ...
      var recipient = (typeof SITE_CONFIG !== 'undefined' && SITE_CONFIG.email) ? SITE_CONFIG.email : 'sales@ningbosiyang.com';
      var fullText = 'To: ' + recipient + '\nSubject: Quote Request – Ningbo Siyang\n\n' + plainText;
      ...
      navigator.clipboard.writeText(fullText).then(function() {
        ...
        if (btn) {
          btn.innerHTML = '<svg class="w-4 h-4 mr-2 text-green-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>Copied!';
          setTimeout(function() {
            btn.innerHTML = '<svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>Copy Quote Details';
          }, 3000);
        }
      }).catch(function() {
        var feedback = document.getElementById('quote-copy-feedback');
