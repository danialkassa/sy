...
console.log('=== FINAL VERIFICATION (REV 2) ===\n');
let allPassed = true;
...
langs.forEach(lang => {
  ...
  report('Translation [' + lang + '] leaf key match', match, match ? trKeys.length + ' keys' : 'missing: ' + enKeys.filter(k => !trKeys.includes(k)).slice(0, 10).join(', '));
});
...
report('Sitemap xhtml:link entries', xhtmlCount === 175, xhtmlCount + ' entries (25 pages x 7)');
report('Sitemap url count', urlCount === 25, urlCount + ' URLs (expected 25)');
