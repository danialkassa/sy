...
function injectHtmlIntoContainer(html, containerId, innerHtml) {
  ...
  // Find the container and inject inside it
  const containerRegex = new RegExp(`(<[^>]*?id=["']${containerId}["'][^>]*?>)([\\s\\S]*?)(</[^>]*?>)`, "g");
  let replaced = false;
  html = html.replace(containerRegex, (match, open, content, close) => {
    replaced = true;
    return open + "\n" + fullInjection + "\n" + close;
  });
  if (!replaced) {
...
function main() {
  ...
  // Testimonials (homepage and about page)
  ...
  processCollectionPage("about/index.html", "cms-testimonials-grid", testimonialCards, { companyData });
